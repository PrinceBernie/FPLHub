...
// STEP 3: Add wallet + leagues + linked teams API calls to dashboard
const Step3Dashboard: React.FC = () => {
  const { user } = useAuth();
  ...
  React.useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Load linked teams
        try {
          const teams = await apiClient.getLinkedTeams();
          console.log('Linked teams loaded:', teams);
          
          // Ensure we have an array
          if (Array.isArray(teams)) {
            setLinkedTeams(teams);
          } else {
            console.warn('getLinkedTeams returned non-array:', teams);
            setLinkedTeams([]);
          }
        } catch (err: any) {
          console.error('Linked teams loading error:', err);
  ...
  return (
    <div className="container-clean py-6">
      ...
      {/* Show actual linked teams if available */}
        ...
        <div className={`flex-1 ${!isMobile ? 'ml-64' : ''}`}>
          <main className={`min-h-screen ${isMobile ? 'pb-20' : ''}`}>
            <Routes>
              ...
              <Route path="/dashboard" element={<Step3Dashboard />} />
